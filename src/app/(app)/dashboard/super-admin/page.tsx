...
import {
  DollarSign, TrendingUp, Building2, CreditCard,
...
export default async function SuperAdminDashboardPage() {
  ...
  if (role !== 'super_admin') redirect(getDashboardRoute(role));
  ...
  const ingresosMes =
    mrrInvoices?.reduce((sum, i) => sum + (i.currency === 'MXN' ? i.amount_cents : 0), 0) ?? 0;

  let mrrEstimated = 0;
  for (const s of mrrSubs ?? []) {
    const plan = s.subscription_plans as {
      price_mxn_cents: number;
      price_mxn_annual_cents: number | null;
    } | null;
    if (!plan) continue;
    const row = s as { billing_interval: string };
    if (row.billing_interval === 'annual') {
      const annual = plan.price_mxn_annual_cents ?? plan.price_mxn_cents * 12;
      mrrEstimated += Math.round(annual / 12);
    } else {
      mrrEstimated += plan.price_mxn_cents;
    }
  }
    ...
    <div className="min-h-screen bg-background">
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="flex flex-wrap items-center gap-2 mb-6">
          <h1 className="font-headline text-2xl font-bold">Dashboard CEO</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
          {[
            ...
            {
              label: 'MRR estimado',
              value: formatMXN(mrrEstimated),
              icon: TrendingUp,
              color: 'text-accent',
            },
            {
              label: 'ARR estimado',
              value: formatMXN(arrEstimated),
              icon: Building2,
              color: 'text-blue-600 dark:text-blue-400',
            },
            { label: 'Activas', value: activeCount ?? 0, icon: CheckCircle2, color: 'text-green-600 dark:text-green-400' },
            { label: 'Trials', value: trialingCount ?? 0, icon: Clock, color: 'text-yellow-600 dark:text-yellow-400' },
            { label: 'Pago pendiente', value: pastDueCount ?? 0, icon: AlertTriangle, color: 'text-red-600 dark:text-red-400' },
            { label: 'Canceladas / exp.', value: canceledCount ?? 0, icon: CreditCard, color: 'text-muted-foreground' },
          ].map((s) => {
            const Icon = s.icon;
            return (
