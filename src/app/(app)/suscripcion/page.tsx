...
import { invoiceStatusLabel } from '@/lib/subscription-status';
...
export default async function SuscripcionPage() {
  ...
  const { data: recentInvoices } = await supabase
    .from('invoices')
    .select('id, amount_cents, status, created_at, currency')
    .eq('user_id', user.id)
    .order('created_at', { ascending: false })
    .limit(3);
  ...
  return (
    <div className="min-h-screen bg-background">
      <main className="container mx-auto px-4 py-8 max-w-2xl">
        <h1 className="font-headline text-xl font-bold mb-6">Mi suscripción</h1>
            ...
            {recentInvoices && recentInvoices.length > 0 && (
              <Card className="bg-card border-border/60">
                <CardHeader className="pb-2">
                  <CardTitle className="font-headline text-base">Últimos cargos</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  {recentInvoices.map((inv) => (
                    <div key={inv.id} className="flex justify-between items-center text-sm py-1 border-b border-border/40 last:border-0">
                      <span className="text-muted-foreground">
                        {new Date(inv.created_at).toLocaleDateString('es-MX')}
                      </span>
                      <span className="font-medium">{formatMXN(inv.amount_cents)}</span>
                      <Badge variant="outline" className="text-xs">
                        {invoiceStatusLabel(inv.status)}
                      </Badge>
                    </div>
                  ))}
                  <Button variant="link" className="px-0 h-auto text-accent cursor-pointer" asChild>
                    <Link href="/facturacion">Ver todo en Facturación →</Link>
                  </Button>
                </CardContent>
              </Card>
            )}
