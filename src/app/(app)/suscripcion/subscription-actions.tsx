...
function processorDisplayName(paymentProcessor: string | null): string {
  if (paymentProcessor === 'stripe') return 'Stripe';
  if (paymentProcessor === 'conekta') return 'Conekta';
  return paymentProcessor ?? 'tu proveedor de pagos';
}
...
export function SubscriptionActions({
  ...
  const processorLabel = processorDisplayName(paymentProcessor);
  const showNonStripeHelp =
    !hasStripe && ['active', 'trialing', 'past_due'].includes(status);
  ...
  return (
    <CardContent className="pt-5 space-y-3">
      {showNonStripeHelp && (
        <p className="text-xs text-muted-foreground text-center px-1 leading-relaxed">
          Tu suscripción se gestiona por <strong>{processorLabel}</strong>. Para cambios de plan, método de pago o
          cancelaciones que no aparezcan aquí, escribe a{' '}
          <a href="mailto:soporte@deeplux.org" className="text-primary underline font-medium cursor-pointer">
            soporte@deeplux.org
          </a>
          .
        </p>
      ...
      {status === 'active' && !cancelAtPeriodEnd && (
        <>
          ...
          <Button
            variant="ghost"
            className="w-full text-muted-foreground hover:text-destructive text-sm cursor-pointer"
      ...
      {cancelAtPeriodEnd && (
        <Button
          variant="outline"
          className="w-full text-primary border-primary/40 cursor-pointer"
